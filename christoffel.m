function [k_vec, t_vec] = christoffel(rho, Cij, omega, kx, ky)
...
%% Input: kx, ky, in-plane wavenumber (given when initialize the plane wave)
%% Output: k_vec, 6*3 matrix, 6 for 3 upgoing 3 downgoing waves. 
%%         3 for kx ky kz components
%% Output: t_vec, polarization matrix, each row is a polarization vector
%%         6*3 matrix, meaning of rwo and column is simular with k_vec.
           
%% First, calculate cijkl*kj*kl
c4 = Ctoc(Cij); % transfer Cij to cijkl

[Q,P,M] = get_coeffmat(c, k1, k2); % Q coef k^0 P coef k^1 M coef k^2
...
kz = transpose(roots (pol));  

[~,indx_sort] = sort(real(p3)); 
